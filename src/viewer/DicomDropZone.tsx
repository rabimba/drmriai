...
import type { SampleDataProgress } from '../utils/sampleDataLoader';
...
let dicomParserPromise: Promise<typeof import('dicom-parser')> | null = null;
let dicomImageLoaderPromise: Promise<typeof import('@cornerstonejs/dicom-image-loader')> | null = null;
let sampleDataLoaderPromise: Promise<typeof import('../utils/sampleDataLoader')> | null = null;

function loadDicomParser() {
  if (!dicomParserPromise) {
    dicomParserPromise = import('dicom-parser');
  }
  return dicomParserPromise;
}

function loadDicomImageLoader() {
  if (!dicomImageLoaderPromise) {
    dicomImageLoaderPromise = import('@cornerstonejs/dicom-image-loader');
  }
  return dicomImageLoaderPromise;
}

function loadSampleDataModule() {
  if (!sampleDataLoaderPromise) {
    sampleDataLoaderPromise = import('../utils/sampleDataLoader');
  }
  return sampleDataLoaderPromise;
}
...
export default function DicomDropZone({ onFilesLoaded }: DicomDropZoneProps) {
  ...
  const processFiles = useCallback(
    async (files: File[]) => {
      ...
      const [dicomParser, { default: cornerstoneDICOMImageLoader }] = await Promise.all([
        loadDicomParser(),
        loadDicomImageLoader(),
      ]);
      ...
      const primaryAxis: AnatomicalPlane =
        primarySeries?.anatomicalPlane && primarySeries.anatomicalPlane !== 'oblique'
          ? primarySeries.anatomicalPlane
          : 'axial';
  ...
  const handleLoadSample = useCallback(async () => {
    ...
    try {
      const { loadSampleData } = await loadSampleDataModule();
  ...
  return (
    ...
    >
      ...
      >
        ...
        {sampleBusy ? sampleLabel : 'Try Sample Knee MRI'}
      </button>
